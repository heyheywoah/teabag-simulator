...
function parseArgs(argv) {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    else if (token === '--span' && i + 1 < argv.length) cfg.span = Math.max(1000, Number(argv[++i]));
...
function printHuman(report) {
  ...
  console.log(`config: seeds=${report.config.seeds} seed_start=${report.config.seedStart} span=${report.config.span}`);
  console.log('metric: overlap_rate = adjacent edge-gap < 0 within same layer+direction');
  console.log('metric: gap stats use edge-gap (curr.x - (prev.x + prev.w)); negatives indicate overlap');
  console.log('');
  ...
  for (let i = 0; i < report.results.length; i++) {
    ...
    console.log(`  FG overlap overall=${formatPct(fg.overall.overlapRate)} right=${formatPct(fg.right.overlapRate)} left=${formatPct(fg.left.overlapRate)} delta(right-left)=${formatPct(fgAsym)}`);
    console.log(`  BG overlap overall=${formatPct(bg.overall.overlapRate)} right=${formatPct(bg.right.overlapRate)} left=${formatPct(bg.left.overlapRate)} delta(right-left)=${formatPct(bgAsym)}`);
    console.log(`  FG gaps mean=${fg.overall.gap.mean.toFixed(2)} p50=${fg.overall.gap.p50.toFixed(2)} p90=${fg.overall.gap.p90.toFixed(2)} p95=${fg.overall.gap.p95.toFixed(2)} min=${fg.overall.gap.min.toFixed(2)} max=${fg.overall.gap.max.toFixed(2)}`);
    console.log(`  BG gaps mean=${bg.overall.gap.mean.toFixed(2)} p50=${bg.overall.gap.p50.toFixed(2)} p90=${bg.overall.gap.p90.toFixed(2)} p95=${bg.overall.gap.p95.toFixed(2)} min=${bg.overall.gap.min.toFixed(2)} max=${bg.overall.gap.max.toFixed(2)}`);
    console.log(`  samples fg_right=${row.sampleCounts.fgRightBuildings} fg_left=${row.sampleCounts.fgLeftBuildings} bg_right=${row.sampleCounts.bgRightBuildings} bg_left=${row.sampleCounts.bgLeftBuildings}`);
    console.log('');
  }
}
